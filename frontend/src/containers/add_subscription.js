    ...
    const PlanList = Plans.map((Plan) => {
        return (
            <form onClick={({ target }) => SetType(target.value)}>
                ...
                <input type="radio" name="sub_type" value={Plan.SUB_TYPE} id="basic" />
                <label htmlFor="basic" style={{ color: "white" }}>   {Plan.SUB_TYPE + " , Maximum " + Plan.NUM_PROFILES + " Profiles , Monthly Bill " + Plan.BILL + "$"} </label>
                <br></br>
                <br></br>
    ...
    const handleSubscription = async event => {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                if (np > mp) {
                    auth.set_ptbd(np - mp);
                    navigate(ROUTES.DELETE_PROFILE);
                } else {
                    navigate(ROUTES.BROWSE); //Successful subscription, moves to netflix browse page
                }
